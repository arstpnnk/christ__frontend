...
export default function ForumList({ navigation }: ForumListProps) {
  ...
  const renderItem = ({ item }: { item: ForumPost }) => (
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate("ForumTopic", {
          postId: item.id,
    ...
    >
      <View style={styles.postContentWrapper}>
        <View style={styles.postHeader}>
          <Image
            source={require("../assets/bubleIcon.png")}
            style={styles.bubble}
          />
          ...
          <TouchableOpacity onPress={() => handleToggleFavorite(item.id)}>
            <Ionicons
              ...
              size={24}
              color={favorites.includes(item.id) ? "#C9E3AC" : "#ccc"}
            />
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  postContainer: {
    ...
    borderRadius: 8,
    marginBottom: 12,
    minHeight: 60, // Минимальная высота для лучшего визуального центрирования
  },
  postContentWrapper: {
    flex: 1,
    justifyContent: "center", // Вертикальное центрирование всего контента
  },
  postHeader: {
    flexDirection: "row",
  ...
  postTitle: {
    ...
    flex: 1,
    marginRight: 10,
    textAlignVertical: "center", // Дополнительное вертикальное выравнивание для текста
  },
  postContent: {
    fontSize: 14,
    color: "#eee",
